...
namespace DriftLock.Core.Input
{
    ...
    public class PlayStationController : IPhysicalController
    {
        ...
        private int _cachedBatteryLevel = 90;
        ...
        public ControllerState GetCurrentState()
        {
            ...
            if (report.ReadStatus == HidDeviceData.ReadStatus.Success)
            {
                ...
                if (data.Length >= 8)
                {
                    int offset = (data[0] == 0x01 && data.Length > 9) ? 1 : 0;
        ...
        private void ParseBatteryReport(byte[] data, int offset)
        {
            if (data.Length > offset + 30)
            {
                ...
                if (data.Length > batIdx)
                {
                    ...
                    int level = Math.Clamp(rawLevel * 10, 0, 100);
                    if (rawLevel <= 5 && Type == ControllerType.DualShock4)
                    {
        ...
        public (string Text, double Percentage, bool IsWireless) GetBatteryInfo()
        {
            ...
            bool isBluetooth = path.Contains("bluetooth") || path.Contains("{00001124") || (!path.Contains("&mi_") && !path.Contains("usb"));
            ...
            int level = Math.Clamp(_cachedBatteryLevel, 0, 100);
            if (level == 0) level = 100;
            string label = $"Battery: {level}%";
            return (label, level / 100.0, true);
        }
